new Vue({
    ...
    data() {
        return {
            ...
            time_show:'',
            adcode:'',
            weatherKey: '529dd15a8cee63c01969d8ab2484958d',
            weatherData: null,
        };
    }, 
    methods: {
        ...
        viewSpot(spot) {
            this.clearMarkers();
            map.flyTo([spot.latitude, spot.longitude], 18, {duration: 1}); 
            const customIcon = L.icon({
                iconUrl: '../src/images/Location.png', 
                iconSize: [40, 40],
                iconAnchor: [20, 40],
            ...
            const marker = L.marker([spot.latitude, spot.longitude], { icon: customIcon });
            console.log(spot)
            marker.bindPopup(`<h3 style="text-align: center"><a href="${spot.src}">${spot.name}</h3>`, {
                offset: [0, -30], 
            });
            marker.addTo(map);
            marker.openPopup();    
